...
질병명·국가명은 자유문장 추출이라 표기가 들쭉날쭉해서(예: "에볼라" vs
...
import db
from .country_risk import COUNTRIES, SIGNAL_TYPE_SEVERITY, _matches as _country_matches
...
def _matched_countries(location: str | None) -> list[str]:
    return [cid for cid, c in COUNTRIES.items() if _country_matches(location, c["aliases"])]
...
def dedupe_events(limit: int = 500) -> list[dict]:
    ...
    for disease_key, members in groups.items():
        ...
        for m in members:
            countries.update(_matched_countries(m["location"]))
